 ...
 * chord.escape.both = [true|false] (default = true) 
 * chord.escape.html = [true|false] (default = false)
 * chord.print.results = [true|false] (default = false)
 * Cannot have chord.escape.both=true and chord.escape.join=lossy for now.
 *
...
public class ThreadEscapeFullAnalysis extends RHSAnalysis<Edge, Edge> {
    ...
    static {
        ...
        useBOTH = System.getProperty("chord.escape.both", "true").equals("true");
        rings = System.getProperty("chord.escape.rings", "false").equals("true");
        ...
        System.out.println("chord.escape.both=" + useBOTH);
        System.out.println("chord.escape.rings=" + rings);
    }
    ...
    @Override
    public boolean mayMerge() { return join != Join.NAIVE; }

    @Override
    public boolean mustMerge() { return join == Join.LOSSY; }

    @Override
    public boolean keepRings() { return rings; }
    ...
    public Set<Pair<Location, Edge>> getInitPathEdges() {
        ...
        Set<Pair<Location, Edge>> initPEs = new ArraySet<Pair<Location, Edge>>(roots.size());
    ...
    public Edge getInvkPathEdge(Quad q, Edge clrPE, jq_Method m, Edge tgtSE) {
        ...
        for (int i = 0, fIdx = 0; i < numArgs; i++) {
            ...
            if (ao.getType().isReferenceType()) {
                ...
                if (aPts != fPts) return null;
                if (aPts != Obj.EMTY && aPts != Obj.ONLY_ESC) allEsc = false;
                fIdx++;
            }
        }
        ...
        if (allEsc) {
            ...
            if (!clrDstHeap.equals(tgtSrcHeap)) return null;
        ...
        DstNode clrDstNode2 = new DstNode(clrDstEnv2, clrDstHeap2, isKill || clrDstNode.isKill, false);
        return new Edge(clrPE.srcNode, clrDstNode2);
    }
    
    /*****************************************************************
     * Intra-procedural transfer functions
     *****************************************************************/
    ...
    class MyQuadVisitor extends QuadVisitor.EmptyVisitor {
        ...
        public void visitPhi(Quad q) {
            ...
            jq_Type t = lo.getType();
            if (t == null || !t.isReferenceType()) return;
            Obj[] iEnv = iDstNode.env;
        ...
        public void visitALoad(Quad q) {
            ...
            if (!((ALoad) op).getType().isReferenceType()) return;
            Obj[] iEnv = iDstNode.env;
        ...
        public void visitGetfield(Quad q) {
            ...
            if (!f.getType().isReferenceType()) return;
            Obj[] iEnv = iDstNode.env;
            ArraySet<FldObj> iHeap = iDstNode.heap;
        ...
        public void visitAStore(Quad q) {
            ...
            Operator op = q.getOperator();
            if (!((AStore) op).getType().isReferenceType()) return;
            Operand rx = AStore.getValue(q);
            if (!(rx instanceof RegisterOperand)) return;
            RegisterOperand bo = (RegisterOperand) AStore.getBase(q);
            RegisterOperand ro = (RegisterOperand) rx;
            Obj[] iEnv = iDstNode.env;
            int rIdx = getIdx(ro);
            Obj rPts = iEnv[rIdx];
            if (rPts == Obj.EMTY) return;
            int bIdx = getIdx(bo);
            Obj bPts = iEnv[bIdx];
            if (bPts == Obj.EMTY) return;
        ...
        public void visitPutfield(Quad q) {
            ...
            if (!(bx instanceof RegisterOperand)) return;
            Obj[] iEnv = iDstNode.env;
            RegisterOperand ro = (RegisterOperand) rx;
            int rIdx = getIdx(ro);
            Obj rPts = iEnv[rIdx];
            ...
            if (bPts == Obj.ONLY_LOC || bPts == Obj.BOTH) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (fo.f == f) {
                        ...
                        if (isLoc == isLoc2 && isEsc == isEsc2) return;
        ...
        public void visitPutstatic(Quad q) {
            ...
            int rIdx = getIdx(ro);
            Obj rPts = iEnv[rIdx];
            if (rPts == Obj.ONLY_ESC || rPts == Obj.EMTY) return;
            oDstNode = reset(iDstNode);
        }
        @Override
        public void visitGetstatic(Quad q) {
            ...
            int lIdx = getIdx(lo);
            if (iEnv[lIdx] == Obj.ONLY_ESC) return;
            Obj[] oEnv = copy(iEnv);
    ...
    private static Obj getPtsFromHeap(Obj bPts, jq_Field f, ArraySet<FldObj> heap) {
        if (bPts == Obj.EMTY || bPts == Obj.ONLY_ESC) return bPts;
        Obj pts = null;
        int n = heap.size();
        for (int i = 0; i < n; i++) {
    ...
    private static DstNode reset(DstNode in) {
        ...
        if (change) {
            ...
            if (in.isKill && in.heap.isEmpty()) return in;
            oEnv = iEnv;
        }
