 ...
 * at program point p is of the form <tt>v = null</tt>.
 *
 * @author Mayur Naik (mhn@cs.stanford.edu)
 */
@Chord(
	name = "PnilValAsgnInst",
	sign = "P0,V0:P0_V0"
)
public class RelPnilValAsgnInst extends ProgramRel {
	public void fill() {
		/*
		...
		for (Type type : program.getTypes()) {
			for (Method method : type.getMethods()) {
				...
				for (Inst inst : cfg.getNodes()) {
					if (inst instanceof NilValAsgnInst) {
						NilValAsgnInst iVal = (NilValAsgnInst) inst;
