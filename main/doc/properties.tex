...
On the command-line via the ``\code{-D<key>=<val>}" format.
...
Via a user-defined file specified by property \code{chord.props.file}.
...
Via provided file \code{chord.properties} in Chord's main directory.
...
%{\tt true}
%{\bf Note:} This functionality is useful for debugging Datalog programs using the {\tt debug} target provided in file {\tt build.xml} in Chord's main directory (see Section \ref{sec:tuning-datalog-analysis}).
