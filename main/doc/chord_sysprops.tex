...
{\bf Type:} [\code{dynamic}$|$\code{rta}$|$\code{rta_reflect}$|$\code{cha}] \\
{\bf Description:} Algorithm to compute analysis scope (i.e., reachable classes and methods).  Current choices include \code{dynamic} (dynamic analysis), \code{rta} (static Rapid Type Analysis ignoring reflection), \code{rta_reflect} (static Rapid Type Analysis modeling a common reflection pattern), and \code{cha} (static Class Hierarchy Analysis). \\
{\bf Default value:} \code{rta}

\item
\code{chord.ch.dynamic} \\
{\bf Type:} bool \\
{\bf Description:} Compute the set of all classes loaded by the JVM while running the given program, and exclude classes not in this set (and their subclasses) while building the class hierarchy.  This property is relevant only if \code{chord.scope.kind} is \code{rta_reflect} or \code{cha}, since only these two scope computing algorithms query the class hierarchy. \\
{\bf Default value:} \code{false} 
