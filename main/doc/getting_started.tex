...
If either of them is set to \code{true}, then you will also need a utility like GNU Make (to run the
...
Each ``\code{-D<key>=<value>}" argument above sets the system property named \code{<key>} to value
\code{<value>}.  The only way to specify inputs to Chord is via system properties; there is no
