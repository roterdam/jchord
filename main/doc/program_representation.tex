...
of a set of local variables and a set of basic blocks.  Each basic
...
packages \javadoc{chord.program}{chord/program/package-tree.html}, \javadoc{joeq.Class}{joeq/Class/package-tree.html},
and \javadoc{joeq.Compiler.Quad}{joeq/Compiler/Quad/package-tree.html}.
...
class \javadoc{joeq.Class.jq_Method}{joeq/Class/jq_Method.html}.  Components of the method (e.g.,
its control-flow graph) can be accessed using the following API provided
by that class.
...
The control-flow graph (CFG) of each method consists of a set of local
variables of the method, called a ``register factory'', and a graph
whose nodes are basic blocks and whose edges denote flow of control
...
The register factory contains one variable per argument of the method
and one variable per temporary declared in the method body.
Temporaries include those declared by programmers as well as those
generated by Joeq.  The reason for the latter is that the quadcode
representation is register-based and is constructed from Java
...
The register factory is a unique object
