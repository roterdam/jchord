...
/**
 * Functionality for transforming a trace of events generated by
 * an instrumented program's execution containing BEF_NEW and
 * AFT_NEW events into a trace containing NEW events.
 * 
 * In the original trace, it is not possible to get the address of
 * the object allocated by a NEW bytecode instruction immediately
 * after that instruction has executed.  JVMs typically forbid
 * accessing the just allocated object until its constructor has
 * finished executing.  Thus, the earliest legal point at which
 * the object's address may be obtained is typically after several
 * other bytecode instructions have been executed.
 * 
 * To remedy this problem, the original trace generates the
 * "BEF_NEW h t" event just before the NEW instruction uniquely
 * identified by h is executed by thread t, and the
 * "AFT_NEW h t o" event just after the constructor of the object
 * allocated by that NEW instruction has finished executing.
 * The h and t fields in the AFT_NEW event are used by this trace
 * transformer to find the matching BEF_NEW event.  It then
 * replaces that BEF_NEW event with a "NEW h t o" event and
 * removes the AFT_NEW event.  Thus, the resulting trace contains
 * only NEW events, no BEF_NEW or AFT_NEW events.
 * 
